    ...
    def forward(self, x, timestep=0.5, scale_list=[8, 4, 2, 1], training=False, fastmode=True, ensemble=False):
        ...
        for i in range(5):
            if flow is None:
                ...
                target_shape = warped_img0.shape[2:]
                wf0 = F.interpolate(wf0, size=target_shape, mode='bilinear', align_corners=False)
                wf1 = F.interpolate(wf1, size=target_shape, mode='bilinear', align_corners=False)
                timestep_resized = F.interpolate(timestep, size=target_shape, mode='nearest')
                mask_resized = F.interpolate(mask, size=target_shape, mode='bilinear', align_corners=False)
                feat_resized = F.interpolate(feat, size=target_shape, mode='bilinear', align_corners=False)
                
                fd, m0, feat = block[i](torch.cat((warped_img0[:, :3], warped_img1[:, :3], wf0, wf1, timestep_resized, mask_resized, feat_resized), 1), flow, scale=scale_list[i])
                if ensemble:
                    print("warning: ensemble is not supported since RIFEv4.21")
                else:
                    mask = m0
